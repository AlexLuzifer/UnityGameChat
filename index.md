...
После ввода сообщения и отправки его путем нажатия на Enter(или любым другим способом, который запустит событие OnSubmit у поля ввода), сообщение добавится в чат.
...
Для добавления сообщения полученного с сервера, необходимо получить объект **Messenger** и вызвать метод **AddMessage(string message)**.
